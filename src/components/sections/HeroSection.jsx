...
const HeroSection = () => {
  ...
  // Initialize isMobile correctly - check on mount
  ...
  return (
    <div className="relative min-h-screen overflow-visible bg-white" style={{ backgroundColor: '#FFFFFF', background: '#FFFFFF' }}>
        ...
        <div className="relative w-full max-w-[1500px] mx-auto grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div className="relative z-30 text-center lg:text-left">
            ...
            >
              ...
              <span className="px-4 py-1 rounded-full border border-[#191919]/20 text-[#191919] text-xs font-semibold tracking-[0.3em] uppercase bg-white/70 backdrop-blur">
            ...
            >
              <motion.p
                className="text-2xl md:text-[50px] font-black text-[#191919] max-w-4xl mx-auto lg:ml-0 leading-tight tracking-tight mb-6"
                ...
                <span className="relative inline-flex items-end h-[1.1em] w-[16ch] align-baseline">
                  <AnimatePresence mode="wait" initial={false}>
                    <motion.span
                      ...
                      exit={isMobile ? { opacity: 0, y: 0 } : { opacity: 0, y: -12 }}
                      transition={isMobile ? { duration: 0.2 } : { duration: 0.35, ease: 'easeOut' }}
                      className="absolute left-0 top-2 w-full capitalize bg-gradient-to-r from-primary to-[#f33f29] bg-clip-text text-transparent"
                    >
            ...
            <motion.div
              ...
              transition={isMobile ? { duration: 0 } : { delay: 1.1, duration: 0.8, ease: 'easeOut' }}
            >
              <motion.div
            ...
            <div className="mt-12 flex flex-wrap justify-center lg:justify-start gap-3">
              {heroBadges.map((badge) => (
                <span
                  key={badge}
                  className="px-4 py-2 rounded-2xl border border-[#191919]/15 bg-white/80 backdrop-blur text-sm font-semibold text-[#191919]/80 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-primary/40 hover:bg-white hover:text-primary shadow-sm hover:shadow-lg"
                >
                  {badge}
                </span>
              ))}
          ...
          <div className="relative hidden lg:flex items-center justify-center">
            <div className="relative rounded-[36px] border border-white/10 p-10 w-full">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <img src="/newlogo.png" alt="Advertio logo" className="h-5 object-contain mb-1" loading="now" />
                  <p className="text-3xl font-black text-[#191919]">Advertio Launch Sprint</p>
                </div>
                <div className="text-right">
                  <span className="text-4xl font-black text-primary">+128%</span>
                  <p className="text-xs text-[#191919]/60 uppercase tracking-[0.4em]">Revenue</p>
                </div>
              </div>
                ...
                <svg viewBox="0 0 280 100" className="w-full h-28">
                  <defs>
                    <linearGradient id="heroLine" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stopColor="#191919" stopOpacity="0.4" />
                      <stop offset="100%" stopColor="#f33f29" stopOpacity="0.8" />
                    </linearGradient>
                  </defs>
                  <motion.polyline
                    fill="none"
                    ...
                    initial={isMobile ? { pathLength: 1, opacity: 1 } : { pathLength: 0, opacity: 0.4 }}
                  ...
                  {staticHeroLinePoints.map((point, idx) => (
                    <motion.circle
                      ...
                      transition={isMobile ? { duration: 0 } : { duration: 0.4, delay: 0.1 * idx }}
                    />
                  ))}
                </svg>
              </div>
              ...
              <div className="space-y-4">
                {staticHeroSignals.map((signal) => (
                  <div key={`static-signal-${signal.label}`}>
                    <div className="flex items-center justify-between text-sm font-semibold text-[#191919] mb-1">
                      <span>{signal.label}</span>
                      <span>{signal.progress}%</span>
                    </div>
                    <div className="h-2 rounded-full bg-[#191919]/10 overflow-hidden">
                      <motion.div
                        className="h-full rounded-full bg-gradient-to-r from-[#191919] to-primary"
                        initial={isMobile ? { width: `${signal.progress}%` } : { width: 0 }}
                ...
                <AnimatePresence mode="wait">
                  {currentHeroMode.highlights.map((item) => (
                    <motion.div
                      ...
                      className="p-4 rounded-2xl border border-[#191919]/10 bg-white/70"
                    >
                      <p className="text-xs uppercase tracking-[0.3em] text-[#191919]/60 mb-2">{item.label}</p>
                      <p className="text-2xl font-black text-[#191919]">{item.value}</p>
                      <p className="text-sm text-[#191919]/70">{item.detail}</p>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
      ...
      >
        <svg className="w-6 h-6 text-[#191919]/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
